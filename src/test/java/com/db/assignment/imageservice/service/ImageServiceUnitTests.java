...
public class ImageServiceUnitTests {
    ...
    public void whenValidRequestAndOptimisedImagePresentInS3_thenReturnOptimisedImageUrl(){
        when(imageTypeStrategyFactory.findStrategy(Mockito.any())).thenReturn(getThumbnailStrategy());
        when(s3StoreRepo.getOptimisedImageFromS3(Mockito.any())).thenReturn(getOptionalExternalImageResponseDto("/optimised/image"));
    ...
    public void whenOptimisedImageNotPresentButOriginalImagePresentInS3_thenOptimiseOriginalImageAndStoreInS3ThenReturn(){
        when(imageTypeStrategyFactory.findStrategy(Mockito.any())).thenReturn(getThumbnailStrategy());
        when(s3StoreRepo.getOptimisedImageFromS3(Mockito.any())).thenReturn(getOptionalExternalImageResponseDto(""));
        when(s3OperationService.getOriginalImageURL(Mockito.any())).thenReturn("/url/to/fetch/from/s3");
        when(s3StoreRepo.getOriginalImageFromS3(Mockito.any())).thenReturn(getOptionalExternalImageResponseDto("/original/from/s3"));
        when(s3StoreRepo.optimise(Mockito.any())).thenReturn(getOptionalExternalImageResponseDto("/optimised/from/s3"));
        when(s3StoreRepo.save(Mockito.any())).thenReturn(getOptionalExternalImageResponseDto("saved/optimised/from/s3"));
    ...
    public void whenOriginalImageNotPresentInS3ButPresentInSource_thenDownloadAndOptimiseAndReturnOriginalImageUrlFromSource(){
        ...
        when(s3StoreRepo.getOptimisedImageFromS3(Mockito.any())).thenReturn(getOptionalExternalImageResponseDto(""));
        when(s3OperationService.getOriginalImageURL(Mockito.any())).thenReturn("/url/to/fetch/from/s3");
        when(s3StoreRepo.getOriginalImageFromS3(Mockito.any())).thenReturn(getOptionalExternalImageResponseDto(""));
        when(sourceStoreRepo.getOriginalImageFromSource(Mockito.any())).thenReturn("/optimised/from/source");
        when(s3StoreRepo.optimise(Mockito.any())).thenReturn(getOptionalExternalImageResponseDto("/optimised/from/source"));
        when(s3StoreRepo.save(Mockito.any())).thenReturn(getOptionalExternalImageResponseDto("saved/optimised/from/source"));
